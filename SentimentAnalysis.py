...
(X_train, Y_train), (X_test, Y_test) = imdb.load_data(          # importing our dataset
    path="imdb.npz",
    num_words=None,
    skip_top=0,
    maxlen=None,
...
review_len_train = []
review_len_test = []
for i, ii, in zip(X_train, X_test):     # word count in comments
...
#decoded_review = whatItSay(2)                  # trying example
...
def good_comment():
    ...
    for i in range(0,24999): 
        ...
        if(Y_train[random_number]==1):
            ...
            text_box.insert('1.0', whatItSay(0))
...
ws = Tk()
ws.title('IMDB Sentiment Analysis')
ws.geometry('900x600')
ws.config(bg='#124272')
